...
def prostoj_klyuch(s):
    # Кириллица переводится в латиницу — ровно как в `sobrat-tablicy.py`.
    # Иначе «Козарак» и «Kozarak» — два ключа, а «ј» вне диапазона «а-я»
    # и вылетает разделителем: «Винарија» даёт «винари-а».
    s = latinicej(s.lower())
...
def klyuch_hozyaistva(imya):
    ...
    Ключи обязаны совпадать с остальными таблицами, иначе рејон не с чем
    будет связать. Поэтому и кириллица переводится в латиницу здесь так же,
    как там: см. `prostoj_klyuch`.
    """
...
def po_mestu(gde, karty):
    ...
    for uroven in UROVNI:
        ...
        if len(rejony) > 1:
            ...
            # Спор общин решает кадастровая община: рејон и определён
            # перечнем катастарских општина, а список общин — это его
            # пересказ. Зрењанин, Нови Бечеј, Кањижа и Дољевац названы
            # в перечне ровно одного рејона, и это не догадка, а закон.
            # Выбирать позволено только среди тех рејонов, которые община
            # и так назвала: иначе сюда влезет село-однофамилец издалека.
            uzhe = {p[0] for p, _ in popadaniya("selo")} & rejony
            if len(uzhe) != 1:
                return None, None, ""
            rejony = uzhe
            nashlos = [x for x in popadaniya("selo") if x[0][0] in rejony]
        rejon = rejony.pop()
